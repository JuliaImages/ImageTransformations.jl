...
and the `img[tform(x)]` is inbound. In the case `tform(x)` maps
to indices outside the original `img`, the value / extrapolation
scheme denoted by the optional parameter `fill` (which defaults
to `NaN` if the element type supports it, and `0` otherwise) is
used to indicate locations for which `tform(x)` was outside the
